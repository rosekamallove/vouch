      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-8 space-y-8">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 bg-muted rounded-md px-3 py-1">
              <code className="text-xs font-mono text-foreground">{collectUrl}</code>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: "Approved", value: approved.length, highlight: false },
            { label: "Rejected", value: rejected.length, highlight: false },
          ].map(({ label, value, highlight }) => (
            <div
              key={label}
              className={`rounded-lg border px-4 py-3 ${highlight ? "border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/30" : ""}`}
            >
        ...
        {/* SECONDARY: API */}
        <section className="space-y-3">
          <h2 className="text-base font-semibold text-muted-foreground">API Access</h2>
